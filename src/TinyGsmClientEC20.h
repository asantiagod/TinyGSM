/**
 * @file       TinyGsmClientEC20.h
 * @author     Volodymyr Shymanskyy, Manoj Sahukar
 * @license    LGPL-3.0
 * @copyright  Copyright (c) 2016 Volodymyr Shymanskyy, Manoj Sahukar
 * @date       Jan 2023
 */

#ifndef SRC_TinyGsmClientEC20_H_
#define SRC_TinyGsmClientEC20_H_
...
class TinyGsmEC20 : public TinyGsmModem<TinyGsmEC20>,
   ...
   */
 public:
  class GsmClientEC20 : public GsmClient {
  ...
  /*
  class GsmClientSecureBG96 : public GsmClientEC20
  {
  public:
    GsmClientSecure() {}
  ...
  */
   ...
   */
 public:
  explicit TinyGsmEC20(Stream& stream) : stream(stream) {
 ...
 public:
  String getNetworkModes() {
    sendAT(GF("+QCFG=?"));
    if (waitResponse(GF(GSM_NL "+QCFG:")) != 1) { return ""; }
  ...
  int16_t getNetworkMode() {
    ...
    if (waitResponse(GF(GSM_NL "+QCFG:")) != 1) { return false; }
  ...
  bool setNetworkMode(uint8_t mode) {
    sendAT(GF("+QCFG=\"nwscanmode\","), mode);
    return waitResponse() == 1;
  }
 ...
 protected:
  ...
  bool isGprsConnectedImpl() {
    ...
    if (waitResponse(GF(GSM_NL "+CGATT: 1")) != 1) { return false; }
    waitResponse();

    sendAT(GF("+CGPADDR=1"));  // Inquire Socket PDP address
    // sendAT(GF("+CGPADDR=1")); // Show PDP address
    if (waitResponse() != 1) { return false; }
...
#endif  // SRC_TinyGsmClientEC20_H_
